...
## <a name="considerations-for-implementing-caching-in-azure"></a>在 Azure 中实现缓存的注意事项
...
Redis 支持读取和写入操作。 在 Redis 中，写入操作会定期存储在本地快照文件或仅限附加的日志文件中，因此可在系统故障时得到保护。 而其他许多缓存（应被视为暂时性数据存储）中并非如此。
...
有关详细信息，请参阅 [Transactions in Redis](https://stackexchange.github.io/StackExchange.Redis/Transactions)（Redis 中的事务）。
