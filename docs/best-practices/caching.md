...
<span data-ttu-id="28e15-113">在缓存数据时，分布式应用程序通常会实施以下一种或两种策略：</span><span class="sxs-lookup"><span data-stu-id="28e15-113">Distributed applications typically implement either or both of the following strategies when caching data:</span></span>
...
### <a name="decide-when-to-cache-data"></a><span data-ttu-id="28e15-148">确定何时缓存数据</span><span class="sxs-lookup"><span data-stu-id="28e15-148">Decide when to cache data</span></span>
...
### <a name="invalidate-data-in-a-client-side-cache"></a><span data-ttu-id="28e15-217">使客户端缓存中的数据失效</span><span class="sxs-lookup"><span data-stu-id="28e15-217">Invalidate data in a client-side cache</span></span>
...
### <a name="protect-cached-data"></a><span data-ttu-id="28e15-286">保护缓存的数据</span><span class="sxs-lookup"><span data-stu-id="28e15-286">Protect cached data</span></span>
...
## <a name="considerations-for-implementing-caching-in-azure"></a><span data-ttu-id="28e15-303">在 Azure 中实现缓存的注意事项</span><span class="sxs-lookup"><span data-stu-id="28e15-303">Considerations for implementing caching in Azure</span></span>
...
<span data-ttu-id="28e15-371">Redis 专门注重于提供数据快速访问，设计为在受信任的环境中运行，且只能由受信任的客户端访问。</span><span class="sxs-lookup"><span data-stu-id="28e15-371">Redis is focused purely on providing fast access to data, and is designed to run inside a trusted environment that can be accessed only by trusted clients.</span></span> <span data-ttu-id="28e15-372">Redis 支持基于密码身份验证的有限安全模型。</span><span class="sxs-lookup"><span data-stu-id="28e15-372">Redis supports a limited security model based on password authentication.</span></span> <span data-ttu-id="28e15-373">（可以完全删除身份验证，但不建议这样做。）</span><span class="sxs-lookup"><span data-stu-id="28e15-373">(It is possible to remove authentication completely, although we don't recommend this.)</span></span>
...
## <a name="caching-session-state-and-html-output"></a><span data-ttu-id="28e15-402">缓存会话状态和 HTML 输出</span><span class="sxs-lookup"><span data-stu-id="28e15-402">Caching session state and HTML output</span></span>
...
## <a name="building-a-custom-redis-cache"></a><span data-ttu-id="28e15-417">构建自定义 Redis 缓存</span><span class="sxs-lookup"><span data-stu-id="28e15-417">Building a custom Redis cache</span></span>
...
<span data-ttu-id="28e15-420">你在实施复制时可能需要创建多个 VM 作为主节点和从属节点，这可能是一个复杂的过程。</span><span class="sxs-lookup"><span data-stu-id="28e15-420">This is a potentially complex process because you might need to create several VMs to act as master and subordinate nodes if you want to implement replication.</span></span> <span data-ttu-id="28e15-421">此外，如果想要创建群集，需要多个主服务器和从属服务器。</span><span class="sxs-lookup"><span data-stu-id="28e15-421">Furthermore, if you wish to create a cluster, then you need multiple masters and subordinate servers.</span></span> <span data-ttu-id="28e15-422">一个可以提供高度可用性和伸缩性，并且至少包含 6 个 VM 并组织成 3 对主/从服务器（一个群集必须至少包含 3 个主节点）的精简群集复制拓扑。</span><span class="sxs-lookup"><span data-stu-id="28e15-422">A minimal clustered replication topology that provides a high degree of availability and scalability comprises at least six VMs organized as three pairs of master/subordinate servers (a cluster must contain at least three master nodes).</span></span>

<span data-ttu-id="28e15-423">每个主/从对应彼此靠近以降低延迟。</span><span class="sxs-lookup"><span data-stu-id="28e15-423">Each master/subordinate pair should be located close together to minimize latency.</span></span> <span data-ttu-id="28e15-424">但如果想要找出靠近的应用程序（该应用程序很可能会使用缓存数据），每一组对可以在位于不同区域的不同 Azure 数据中心运行。</span><span class="sxs-lookup"><span data-stu-id="28e15-424">However, each set of pairs can be running in different Azure datacenters located in different regions, if you wish to locate cached data close to the applications that are most likely to use it.</span></span> <span data-ttu-id="28e15-425">有关生成和配置作为 Azure VM 运行的 Redis 节点的示例，请参阅 [Running Redis on a CentOS Linux VM in Azure](https://blogs.msdn.microsoft.com/tconte/2012/06/08/running-redis-on-a-centos-linux-vm-in-windows-azure/)（在 Azure 中的 CentOS Linux VM 上运行 Redis）。</span><span class="sxs-lookup"><span data-stu-id="28e15-425">For an example of building and configuring a Redis node running as an Azure VM, see [Running Redis on a CentOS Linux VM in Azure](https://blogs.msdn.microsoft.com/tconte/2012/06/08/running-redis-on-a-centos-linux-vm-in-windows-azure/).</span></span>

> [!NOTE]
> <span data-ttu-id="28e15-426">请注意，如果以这种方式实施自己的 Redis 缓存，则需要负责监视、管理和保护服务。</span><span class="sxs-lookup"><span data-stu-id="28e15-426">Please note that if you implement your own Redis cache in this way, you are responsible for monitoring, managing, and securing the service.</span></span>

## <a name="partitioning-a-redis-cache"></a><span data-ttu-id="28e15-427">将 Redis 缓存分区</span><span class="sxs-lookup"><span data-stu-id="28e15-427">Partitioning a Redis cache</span></span>

<span data-ttu-id="28e15-428">将缓存分区涉及到在多台计算机之间拆分缓存。</span><span class="sxs-lookup"><span data-stu-id="28e15-428">Partitioning the cache involves splitting the cache across multiple computers.</span></span> <span data-ttu-id="28e15-429">此结构使用单个缓存服务器，可以提供多种优势，包括：</span><span class="sxs-lookup"><span data-stu-id="28e15-429">This structure gives you several advantages over using a single cache server, including:</span></span>

- <span data-ttu-id="28e15-430">创建的缓存比单个服务器上存储的缓存要大得多。</span><span class="sxs-lookup"><span data-stu-id="28e15-430">Creating a cache that is much bigger than can be stored on a single server.</span></span>
- <span data-ttu-id="28e15-431">将数据分散到多个服务器，从而提高可用性。</span><span class="sxs-lookup"><span data-stu-id="28e15-431">Distributing data across servers, improving availability.</span></span> <span data-ttu-id="28e15-432">如果一台服务器发生故障或不可访问，该服务器保存的数据将不可用，但剩余服务器上的数据仍可访问。</span><span class="sxs-lookup"><span data-stu-id="28e15-432">If one server fails or becomes inaccessible, the data that it holds is unavailable, but the data on the remaining servers can still be accessed.</span></span> <span data-ttu-id="28e15-433">对于缓存而言这并不重要，因为缓存数据只是数据库中暂时保存的数据副本。</span><span class="sxs-lookup"><span data-stu-id="28e15-433">For a cache, this is not crucial because the cached data is only a transient copy of the data that's held in a database.</span></span> <span data-ttu-id="28e15-434">不可访问的服务器上的缓存数据可以改为在不同的服务器上缓存。</span><span class="sxs-lookup"><span data-stu-id="28e15-434">Cached data on a server that becomes inaccessible can be cached on a different server instead.</span></span>
- <span data-ttu-id="28e15-435">在服务器之间分散负载，从而提高性能和伸缩性。</span><span class="sxs-lookup"><span data-stu-id="28e15-435">Spreading the load across servers, thereby improving performance and scalability.</span></span>
- <span data-ttu-id="28e15-436">将数据放置在靠近用户访问的地理位置以降低延迟。</span><span class="sxs-lookup"><span data-stu-id="28e15-436">Geolocating data close to the users that access it, thus reducing latency.</span></span>

<span data-ttu-id="28e15-437">对于缓存，最常见的分区形式是分片。</span><span class="sxs-lookup"><span data-stu-id="28e15-437">For a cache, the most common form of partitioning is sharding.</span></span> <span data-ttu-id="28e15-438">在此策略中，每个分区（或分片）本身是一个 Redis 缓存。</span><span class="sxs-lookup"><span data-stu-id="28e15-438">In this strategy, each partition (or shard) is a Redis cache in its own right.</span></span> <span data-ttu-id="28e15-439">数据使用分片逻辑定向到特定的分区，该逻辑可以使用各种方法来分布数据。</span><span class="sxs-lookup"><span data-stu-id="28e15-439">Data is directed to a specific partition by using sharding logic, which can use a variety of approaches to distribute the data.</span></span> <span data-ttu-id="28e15-440">[Sharding pattern](../patterns/sharding.md)（分片模式）提供了有关实施分片的详细信息。</span><span class="sxs-lookup"><span data-stu-id="28e15-440">The [Sharding pattern](../patterns/sharding.md) provides more information about implementing sharding.</span></span>

<span data-ttu-id="28e15-441">若要在 Redis 缓存中实施分区，可以采用以下方法之一：</span><span class="sxs-lookup"><span data-stu-id="28e15-441">To implement partitioning in a Redis cache, you can take one of the following approaches:</span></span>

- <span data-ttu-id="28e15-442">*服务器端查询路由。*</span><span class="sxs-lookup"><span data-stu-id="28e15-442">*Server-side query routing.*</span></span> <span data-ttu-id="28e15-443">使用此方法时，客户端应用程序会将请求发送到构成缓存的任何 Redis 服务器（可能是最靠近的服务器）。</span><span class="sxs-lookup"><span data-stu-id="28e15-443">In this technique, a client application sends a request to any of the Redis servers that comprise the cache (probably the closest server).</span></span> <span data-ttu-id="28e15-444">每个 Redis 服务器将存储用于描述它所保存的分区的元数据，同时还包含有关哪些分区位于其他服务器上的信息。</span><span class="sxs-lookup"><span data-stu-id="28e15-444">Each Redis server stores metadata that describes the partition that it holds, and also contains information about which partitions are located on other servers.</span></span> <span data-ttu-id="28e15-445">Redis 服务器检查客户端请求。</span><span class="sxs-lookup"><span data-stu-id="28e15-445">The Redis server examines the client request.</span></span> <span data-ttu-id="28e15-446">如果可以在本地解决，则执行请求的操作。</span><span class="sxs-lookup"><span data-stu-id="28e15-446">If it can be resolved locally, it will perform the requested operation.</span></span> <span data-ttu-id="28e15-447">否则将请求转发到相应的服务器。</span><span class="sxs-lookup"><span data-stu-id="28e15-447">Otherwise it will forward the request on to the appropriate server.</span></span> <span data-ttu-id="28e15-448">此模型是通过 Redis 群集实施的，Redis 网站上的 [Redis 群集教程](https://redis.io/topics/cluster-tutorial)页上提供了更详细的说明。</span><span class="sxs-lookup"><span data-stu-id="28e15-448">This model is implemented by Redis clustering, and is described in more detail on the [Redis cluster tutorial](https://redis.io/topics/cluster-tutorial) page on the Redis website.</span></span> <span data-ttu-id="28e15-449">Redis 群集对客户端应用程序而言是透明的，其他 Redis 服务器可以添加到群集（数据将重新分区），而无需重新配置客户端。</span><span class="sxs-lookup"><span data-stu-id="28e15-449">Redis clustering is transparent to client applications, and additional Redis servers can be added to the cluster (and the data re-partitioned) without requiring that you reconfigure the clients.</span></span>
- <span data-ttu-id="28e15-450">*客户端分区。*</span><span class="sxs-lookup"><span data-stu-id="28e15-450">*Client-side partitioning.*</span></span> <span data-ttu-id="28e15-451">在此模型中，客户端应用程序包含将请求路由到适当 Redis 服务器的逻辑（可能以库的形式）。</span><span class="sxs-lookup"><span data-stu-id="28e15-451">In this model, the client application contains logic (possibly in the form of a library) that routes requests to the appropriate Redis server.</span></span> <span data-ttu-id="28e15-452">这种方法可以配合 Azure Redis 缓存使用。</span><span class="sxs-lookup"><span data-stu-id="28e15-452">This approach can be used with Azure Redis Cache.</span></span> <span data-ttu-id="28e15-453">创建多个 Azure Redis 缓存（每个数据分区一个缓存），并实施将请求路由到正确缓存的客户端逻辑。</span><span class="sxs-lookup"><span data-stu-id="28e15-453">Create multiple Azure Redis Caches (one for each data partition) and implement the client-side logic that routes the requests to the correct cache.</span></span> <span data-ttu-id="28e15-454">如果分区方案发生更改（例如，如果已创建其他 Azure Redis 缓存），则可能需要重新配置客户端应用程序。</span><span class="sxs-lookup"><span data-stu-id="28e15-454">If the partitioning scheme changes (if additional Azure Redis Caches are created, for example), client applications might need to be reconfigured.</span></span>
- <span data-ttu-id="28e15-455">*代理辅助分区。*</span><span class="sxs-lookup"><span data-stu-id="28e15-455">*Proxy-assisted partitioning.*</span></span> <span data-ttu-id="28e15-456">在此方案中，客户端应用程序将请求发送到一个知道如何数据分区方式的中间代理服务，然后将请求路由到适当的 Redis 服务器。</span><span class="sxs-lookup"><span data-stu-id="28e15-456">In this scheme, client applications send requests to an intermediary proxy service which understands how the data is partitioned and then routes the request to the appropriate Redis server.</span></span> <span data-ttu-id="28e15-457">此方法也可以配合 Azure Redis 缓存使用；代理服务可以实施为 Azure 云服务。</span><span class="sxs-lookup"><span data-stu-id="28e15-457">This approach can also be used with Azure Redis Cache; the proxy service can be implemented as an Azure cloud service.</span></span> <span data-ttu-id="28e15-458">使用此方法实施服务需要提高复杂性，并且执行请求的时间可能比使用客户端分区更长。</span><span class="sxs-lookup"><span data-stu-id="28e15-458">This approach requires an additional level of complexity to implement the service, and requests might take longer to perform than using client-side partitioning.</span></span>

<span data-ttu-id="28e15-459">Redis 网站上的 [Partitioning: how to split data among multiple Redis instances](https://redis.io/topics/partitioning)（分区：如何在多个 Redis 实例之间拆分数据）页提供了有关使用 Redis 实施分区的更多信息。</span><span class="sxs-lookup"><span data-stu-id="28e15-459">The page [Partitioning: how to split data among multiple Redis instances](https://redis.io/topics/partitioning) on the Redis website provides further information about implementing partitioning with Redis.</span></span>

### <a name="implement-redis-cache-client-applications"></a><span data-ttu-id="28e15-460">实施 Redis 缓存客户端应用程序</span><span class="sxs-lookup"><span data-stu-id="28e15-460">Implement Redis cache client applications</span></span>

<span data-ttu-id="28e15-461">Redis 支持以多种编程语言编写的客户端应用程序。</span><span class="sxs-lookup"><span data-stu-id="28e15-461">Redis supports client applications written in numerous programming languages.</span></span> <span data-ttu-id="28e15-462">如果要使用.NET Framework 构建新的应用程序，建议的方法是使用 StackExchange.Redis 客户端库。</span><span class="sxs-lookup"><span data-stu-id="28e15-462">If you are building new applications by using the .NET Framework, the recommended approach is to use the StackExchange.Redis client library.</span></span> <span data-ttu-id="28e15-463">此库提供 .NET Framework 对象模型，用于抽象连接到 Redis 服务器连接、发送命令和接收响应所需的详细信息。</span><span class="sxs-lookup"><span data-stu-id="28e15-463">This library provides a .NET Framework object model that abstracts the details for connecting to a Redis server, sending commands, and receiving responses.</span></span> <span data-ttu-id="28e15-464">在 Visual Studio 中，它以 NuGet 包的形式提供。</span><span class="sxs-lookup"><span data-stu-id="28e15-464">It is available in Visual Studio as a NuGet package.</span></span> <span data-ttu-id="28e15-465">可以使用同一个库连接到 Azure Redis 缓存，或者 VM 上托管的自定义 Redis 缓存。</span><span class="sxs-lookup"><span data-stu-id="28e15-465">You can use this same library to connect to an Azure Redis Cache, or a custom Redis cache hosted on a VM.</span></span>

<span data-ttu-id="28e15-466">若要连接到 Redis 服务器，可以使用 `ConnectionMultiplexer` 类的静态 `Connect` 方法。</span><span class="sxs-lookup"><span data-stu-id="28e15-466">To connect to a Redis server you use the static `Connect` method of the `ConnectionMultiplexer` class.</span></span> <span data-ttu-id="28e15-467">此方法创建的连接可在客户端应用程序的整个生存期内使用，同一个连接可由多个并发线程使用。</span><span class="sxs-lookup"><span data-stu-id="28e15-467">The connection that this method creates is designed to be used throughout the lifetime of the client application, and the same connection can be used by multiple concurrent threads.</span></span> <span data-ttu-id="28e15-468">每次执行 Redis 操作时，请不要重新连接和断开连接，因为这可能会降低性能。</span><span class="sxs-lookup"><span data-stu-id="28e15-468">Do not reconnect and disconnect each time you perform a Redis operation because this can degrade performance.</span></span>

<span data-ttu-id="28e15-469">可以指定连接参数，例如 Redis 主机的地址和密码。</span><span class="sxs-lookup"><span data-stu-id="28e15-469">You can specify the connection parameters, such as the address of the Redis host and the password.</span></span> <span data-ttu-id="28e15-470">如果使用 Azure Redis 缓存，密码可能是使用 Azure 管理门户针对 Azure Redis 缓存生成的主密钥或辅助密钥。</span><span class="sxs-lookup"><span data-stu-id="28e15-470">If you are using Azure Redis Cache, the password is either the primary or secondary key that is generated for Azure Redis Cache by using the Azure Management portal.</span></span>

<span data-ttu-id="28e15-471">在已连接到 Redis 服务器后，可以在用作缓存的 Redis 数据库上获取句柄。</span><span class="sxs-lookup"><span data-stu-id="28e15-471">After you have connected to the Redis server, you can obtain a handle on the Redis database that acts as the cache.</span></span> <span data-ttu-id="28e15-472">Redis 连接提供了 `GetDatabase` 方法来执行此操作。</span><span class="sxs-lookup"><span data-stu-id="28e15-472">The Redis connection provides the `GetDatabase` method to do this.</span></span> <span data-ttu-id="28e15-473">然后，可以使用 `StringGet` 和 `StringSet` 方法，从缓存中检索项并在缓存中存储数据。</span><span class="sxs-lookup"><span data-stu-id="28e15-473">You can then retrieve items from the cache and store data in the cache by using the `StringGet` and `StringSet` methods.</span></span> <span data-ttu-id="28e15-474">这些方法需要将键用作参数，并返回缓存中具有匹配值的项 (`StringGet`)，或者将项添加到具有此键的缓存 (`StringSet`)。</span><span class="sxs-lookup"><span data-stu-id="28e15-474">These methods expect a key as a parameter, and return the item either in the cache that has a matching value (`StringGet`) or add the item to the cache with this key (`StringSet`).</span></span>

<span data-ttu-id="28e15-475">根据 Redis 服务器的位置，在将请求传输到服务器以及将响应返回给客户端时，许多操作可能会造成一些延迟。</span><span class="sxs-lookup"><span data-stu-id="28e15-475">Depending on the location of the Redis server, many operations might incur some latency while a request is transmitted to the server and a response is returned to the client.</span></span> <span data-ttu-id="28e15-476">StackExchange 库公开了许多方法的异步版本，用于帮助客户端应用程序保持响应。</span><span class="sxs-lookup"><span data-stu-id="28e15-476">The StackExchange library provides asynchronous versions of many of the methods that it exposes to help client applications remain responsive.</span></span> <span data-ttu-id="28e15-477">这些方法支持 .NET Framework 中的[基于任务的异步模式](/dotnet/standard/asynchronous-programming-patterns/task-based-asynchronous-pattern-tap)。</span><span class="sxs-lookup"><span data-stu-id="28e15-477">These methods support the [Task-based Asynchronous pattern](/dotnet/standard/asynchronous-programming-patterns/task-based-asynchronous-pattern-tap) in the .NET Framework.</span></span>

<span data-ttu-id="28e15-478">以下代码片段显示了名为 `RetrieveItem` 的方法。</span><span class="sxs-lookup"><span data-stu-id="28e15-478">The following code snippet shows a method named `RetrieveItem`.</span></span> <span data-ttu-id="28e15-479">其中演示了基于 Redis 和 StackExchange 库的缓存端模式的实现。</span><span class="sxs-lookup"><span data-stu-id="28e15-479">It illustrates an implementation of the cache-aside pattern based on Redis and the StackExchange library.</span></span> <span data-ttu-id="28e15-480">该方法采用字符串键值，并通过调用 `StringGetAsync` 方法（`StringGet` 的异步版本）尝试从 Redis 缓存中检索相应的项。</span><span class="sxs-lookup"><span data-stu-id="28e15-480">The method takes a string key value and attempts to retrieve the corresponding item from the Redis cache by calling the `StringGetAsync` method (the asynchronous version of `StringGet`).</span></span>

<span data-ttu-id="28e15-481">如果找不到该项，则使用 `GetItemFromDataSourceAsync` 方法（这是一个本地方法，它不是 StackExchange 库的一部分）从底层数据源提取该项。</span><span class="sxs-lookup"><span data-stu-id="28e15-481">If the item is not found, it is fetched from the underlying data source using the `GetItemFromDataSourceAsync` method (which is a local method and not part of the StackExchange library).</span></span> <span data-ttu-id="28e15-482">然后，使用 `StringSetAsync` 方法将该项添加到缓存，以便下一次可以更快地检索。</span><span class="sxs-lookup"><span data-stu-id="28e15-482">It's then added to the cache by using the `StringSetAsync` method so it can be retrieved more quickly next time.</span></span>
...
<span data-ttu-id="28e15-483">`StringGet` 和 `StringSet` 方法不是只能检索或存储字符串值。</span><span class="sxs-lookup"><span data-stu-id="28e15-483">The `StringGet` and `StringSet` methods are not restricted to retrieving or storing string values.</span></span> <span data-ttu-id="28e15-484">它们可以采用任何序列化为字节数组的项。</span><span class="sxs-lookup"><span data-stu-id="28e15-484">They can take any item that is serialized as an array of bytes.</span></span> <span data-ttu-id="28e15-485">如果需要保存 .NET 对象，可以将它序列化为字节流，然后使用 `StringSet` 方法将它写入缓存。</span><span class="sxs-lookup"><span data-stu-id="28e15-485">If you need to save a .NET object, you can serialize it as a byte stream and use the `StringSet` method to write it to the cache.</span></span>

<span data-ttu-id="28e15-486">同样地，可以使用 `StringGet` 方法从缓存中读取对象，并将其反序列化为 .NET 对象。</span><span class="sxs-lookup"><span data-stu-id="28e15-486">Similarly, you can read an object from the cache by using the `StringGet` method and deserializing it as a .NET object.</span></span> <span data-ttu-id="28e15-487">以下代码演示了 IDatabase 接口的一组扩展方法（Redis 连接的 `GetDatabase` 方法返回 `IDatabase` 对象），使用这些方法的某些示例代码可以在缓存中读取和写入 `BlogPost` 对象：</span><span class="sxs-lookup"><span data-stu-id="28e15-487">The following code shows a set of extension methods for the IDatabase interface (the `GetDatabase` method of a Redis connection returns an `IDatabase` object),  and some sample code that uses these methods to read and write a `BlogPost` object to the cache:</span></span>
...
<span data-ttu-id="28e15-489">如果客户端应用程序发送了多个异步请求，Redis 将支持命令管道。</span><span class="sxs-lookup"><span data-stu-id="28e15-489">Redis supports command pipelining if a client application sends multiple asynchronous requests.</span></span> <span data-ttu-id="28e15-490">Redis 可以使用同一连接来多路复用请求，而不是按照严格的顺序来接收和响应命令。</span><span class="sxs-lookup"><span data-stu-id="28e15-490">Redis can multiplex the requests using the same connection rather than receiving and responding to commands in a strict sequence.</span></span>

<span data-ttu-id="28e15-491">此方法可以更有效地使用网络来帮助降低延迟。</span><span class="sxs-lookup"><span data-stu-id="28e15-491">This approach helps to reduce latency by making more efficient use of the network.</span></span> <span data-ttu-id="28e15-492">以下代码段演示了并行检索两个客户的详细信息的示例。</span><span class="sxs-lookup"><span data-stu-id="28e15-492">The following code snippet shows an example that retrieves the details of two customers concurrently.</span></span> <span data-ttu-id="28e15-493">该代码将提交两个请求，再执行其他某种处理（未显示），然后等待接收结果。</span><span class="sxs-lookup"><span data-stu-id="28e15-493">The code submits two requests and then performs some other processing (not shown) before waiting to receive the results.</span></span> <span data-ttu-id="28e15-494">缓存对象的 `Wait` 方法类似于 .NET Framework `Task.Wait` 方法：</span><span class="sxs-lookup"><span data-stu-id="28e15-494">The `Wait` method of the cache object is similar to the .NET Framework `Task.Wait` method:</span></span>
...
<span data-ttu-id="28e15-495">有关如何编写可使用 Azure Redis 缓存的客户端应用程序的其他信息，请参阅 [Azure Redis 缓存文档](/azure/azure-cache-for-redis/)。</span><span class="sxs-lookup"><span data-stu-id="28e15-495">For additional information on writing client applications that can the Azure Redis Cache, see the [Azure Cache for Redis documentation](/azure/azure-cache-for-redis/).</span></span> <span data-ttu-id="28e15-496">请在 [StackExchange.Redis](https://github.com/StackExchange/StackExchange.Redis/blob/master/Docs/Basics.md) 处查看详细信息。</span><span class="sxs-lookup"><span data-stu-id="28e15-496">More information is also available at [StackExchange.Redis](https://github.com/StackExchange/StackExchange.Redis/blob/master/Docs/Basics.md).</span></span>

<span data-ttu-id="28e15-497">同一网站上的 [Pipelines and multiplexers](https://stackexchange.github.io/StackExchange.Redis/PipelinesMultiplexers)（管道与多路复用器）页提供了有关使用 Redis 和 StackExchange 库执行异步操作和管道传输的详细信息。</span><span class="sxs-lookup"><span data-stu-id="28e15-497">The page [Pipelines and multiplexers](https://stackexchange.github.io/StackExchange.Redis/PipelinesMultiplexers) on the same website provides more information about asynchronous operations and pipelining with Redis and the StackExchange library.</span></span>

## <a name="using-redis-caching"></a><span data-ttu-id="28e15-498">使用 Redis 缓存</span><span class="sxs-lookup"><span data-stu-id="28e15-498">Using Redis caching</span></span>

<span data-ttu-id="28e15-499">Redis 缓存的最简单用法包括存储键-值对，其中的值是未解释的字符串，该字符串具有任意长度，可以包含任何二进制数据。</span><span class="sxs-lookup"><span data-stu-id="28e15-499">The simplest use of Redis for caching concerns is key-value pairs where the value is an uninterpreted string of arbitrary length that can contain any binary data.</span></span> <span data-ttu-id="28e15-500">（本质上是可视为字符串的字节数组）。</span><span class="sxs-lookup"><span data-stu-id="28e15-500">(It is essentially an array of bytes that can be treated as a string).</span></span> <span data-ttu-id="28e15-501">本文前面的“实施 Redis 缓存客户端应用程序”部分中已演示这种方案。</span><span class="sxs-lookup"><span data-stu-id="28e15-501">This scenario was illustrated in the section Implement Redis Cache client applications earlier in this article.</span></span>

<span data-ttu-id="28e15-502">请注意，键还包含未解释的数据，因此，可以使用任何二进制信息作为键。</span><span class="sxs-lookup"><span data-stu-id="28e15-502">Note that keys also contain uninterpreted data, so you can use any binary information as the key.</span></span> <span data-ttu-id="28e15-503">但键越长，存储花费的空间就越多，执行查找操作所需的时间也越长。</span><span class="sxs-lookup"><span data-stu-id="28e15-503">The longer the key is, however, the more space it will take to store, and the longer it will take to perform lookup operations.</span></span> <span data-ttu-id="28e15-504">为了实现可用性和易维护性，请认真设计键空间并使用有意义（但非详细）的键。</span><span class="sxs-lookup"><span data-stu-id="28e15-504">For usability and ease of maintenance, design your keyspace carefully and use meaningful (but not verbose) keys.</span></span>

<span data-ttu-id="28e15-505">例如，使用类似于“customer:100”的结构化键来表示 ID 为 100 的客户的键，而不是简单地使用“100”。</span><span class="sxs-lookup"><span data-stu-id="28e15-505">For example, use structured keys such as "customer:100" to represent the key for the customer with ID 100 rather than simply "100".</span></span> <span data-ttu-id="28e15-506">使用此方案可以轻松区分存储不同数据类型的值。</span><span class="sxs-lookup"><span data-stu-id="28e15-506">This scheme enables you to easily distinguish between values that store different data types.</span></span> <span data-ttu-id="28e15-507">例如，也可以使用键“orders:100”来表示 ID为 100 的订单的键。</span><span class="sxs-lookup"><span data-stu-id="28e15-507">For example, you could also use the key "orders:100" to represent the key for the order with ID 100.</span></span>

<span data-ttu-id="28e15-508">除了一维二进制字符串以外，Redis 键-值对中的值还可以包含更结构化的信息，包括列表、集（已排序和未排序）和哈希。</span><span class="sxs-lookup"><span data-stu-id="28e15-508">Apart from one-dimensional binary strings, a value in a Redis key-value pair can also hold more structured information, including lists, sets (sorted and unsorted), and hashes.</span></span> <span data-ttu-id="28e15-509">Redis 提供全面的命令集用于处理这些类型，其中的许多命令可以通过 StackExchange 等客户端库用于 .NET Framework 应用程序。</span><span class="sxs-lookup"><span data-stu-id="28e15-509">Redis provides a comprehensive command set that can manipulate these types, and many of these commands are available to .NET Framework applications through a client library such as StackExchange.</span></span> <span data-ttu-id="28e15-510">Redis 网站上的 [An introduction to Redis data types and abstractions](https://redis.io/topics/data-types-intro)（Redis 数据类型和抽象简介）页更详细地概述了这些类型以及可用于处理这些类型的命令。</span><span class="sxs-lookup"><span data-stu-id="28e15-510">The page [An introduction to Redis data types and abstractions](https://redis.io/topics/data-types-intro) on the Redis website provides a more detailed overview of these types and the commands that you can use to manipulate them.</span></span>

<span data-ttu-id="28e15-511">本部分汇总了这些数据类型和命令的一些常见用例。</span><span class="sxs-lookup"><span data-stu-id="28e15-511">This section summarizes some common use cases for these data types and commands.</span></span>

### <a name="perform-atomic-and-batch-operations"></a><span data-ttu-id="28e15-512">执行原子操作和批处理操作</span><span class="sxs-lookup"><span data-stu-id="28e15-512">Perform atomic and batch operations</span></span>

<span data-ttu-id="28e15-513">Redis 支持对字符串值执行一系列原子性“获取和设置”操作。</span><span class="sxs-lookup"><span data-stu-id="28e15-513">Redis supports a series of atomic get-and-set operations on string values.</span></span> <span data-ttu-id="28e15-514">这些操作将删除使用单独的 `GET` 和 `SET` 命令时可能发生的争用风险。</span><span class="sxs-lookup"><span data-stu-id="28e15-514">These operations remove the possible race hazards that might occur when using separate `GET` and `SET` commands.</span></span> <span data-ttu-id="28e15-515">可用的操作包括：</span><span class="sxs-lookup"><span data-stu-id="28e15-515">The operations that are available include:</span></span>

- <span data-ttu-id="28e15-516">`INCR`、`INCRBY`、`DECR` 和 `DECRBY`，用于对整数数字数据值执行原子递增和递减操作。</span><span class="sxs-lookup"><span data-stu-id="28e15-516">`INCR`, `INCRBY`, `DECR`, and `DECRBY`, which perform atomic increment and decrement operations on integer numeric data values.</span></span> <span data-ttu-id="28e15-517">StackExchange 库提供了 `IDatabase.StringIncrementAsync` 和 `IDatabase.StringDecrementAsync` 方法的重载版本，用于执行这些操作并返回存储在缓存中的结果值。</span><span class="sxs-lookup"><span data-stu-id="28e15-517">The StackExchange library provides overloaded versions of the `IDatabase.StringIncrementAsync` and `IDatabase.StringDecrementAsync` methods to perform these operations and return the resulting value that is stored in the cache.</span></span> <span data-ttu-id="28e15-518">以下代码段演示了如何使用这些方法：</span><span class="sxs-lookup"><span data-stu-id="28e15-518">The following code snippet illustrates how to use these methods:</span></span>
...
- <span data-ttu-id="28e15-519">`GETSET` 用于检索与键关联的值，并将其更改为新值。</span><span class="sxs-lookup"><span data-stu-id="28e15-519">`GETSET`, which retrieves the value that's associated with a key and changes it to a new value.</span></span> <span data-ttu-id="28e15-520">StackExchange 库通过 `IDatabase.StringGetSetAsync` 方法使此操作可供使用。</span><span class="sxs-lookup"><span data-stu-id="28e15-520">The StackExchange library makes this operation available through the `IDatabase.StringGetSetAsync` method.</span></span> <span data-ttu-id="28e15-521">以下代码段演示了此方法的示例。</span><span class="sxs-lookup"><span data-stu-id="28e15-521">The code snippet below shows an example of this method.</span></span> <span data-ttu-id="28e15-522">此代码从前一示例返回与键 "data:counter" 关联的当前值。</span><span class="sxs-lookup"><span data-stu-id="28e15-522">This code returns the current value that's associated with the key "data:counter" from the previous example.</span></span> <span data-ttu-id="28e15-523">然后将此键的值重置为零，这些都是同一操作的一部分：</span><span class="sxs-lookup"><span data-stu-id="28e15-523">Then it resets the value for this key back to zero, all as part of the same operation:</span></span>
...
- <span data-ttu-id="28e15-524">`MGET` 和 `MSET` 可以作为单个操作返回或更改一组字符串值。</span><span class="sxs-lookup"><span data-stu-id="28e15-524">`MGET` and `MSET`, which can return or change a set of string values as a single operation.</span></span> <span data-ttu-id="28e15-525">`IDatabase.StringGetAsync` 和 `IDatabase.StringSetAsync` 已重载以支持此功能，如以下示例中所示：</span><span class="sxs-lookup"><span data-stu-id="28e15-525">The `IDatabase.StringGetAsync` and `IDatabase.StringSetAsync` methods are overloaded to support this functionality, as shown in the following example:</span></span>
...
<span data-ttu-id="28e15-526">也可以将多个操作合并成单个 Redis 事务，如本文前面的“Redis 事务和批处理”部分中所述。</span><span class="sxs-lookup"><span data-stu-id="28e15-526">You can also combine multiple operations into a single Redis transaction as described in the Redis transactions and batches section earlier in this article.</span></span> <span data-ttu-id="28e15-527">StackExchange 库通过 `ITransaction` 接口提供事务支持。</span><span class="sxs-lookup"><span data-stu-id="28e15-527">The StackExchange library provides support for transactions through the `ITransaction` interface.</span></span>

<span data-ttu-id="28e15-528">使用 `IDatabase.CreateTransaction` 方法创建 `ITransaction` 对象。</span><span class="sxs-lookup"><span data-stu-id="28e15-528">You create an `ITransaction` object by using the `IDatabase.CreateTransaction` method.</span></span> <span data-ttu-id="28e15-529">使用 `ITransaction` 对象提供的方法调用对事务的命令。</span><span class="sxs-lookup"><span data-stu-id="28e15-529">You invoke commands to the transaction by using the methods provided by the `ITransaction` object.</span></span>

<span data-ttu-id="28e15-530">`ITransaction` 接口可用于访问 `IDatabase` 接口所访问的类似一组方法，不过，所有方法是异步的。</span><span class="sxs-lookup"><span data-stu-id="28e15-530">The `ITransaction` interface provides access to a set of methods that's similar to those accessed by the `IDatabase` interface, except that all the methods are asynchronous.</span></span> <span data-ttu-id="28e15-531">这意味着，这些方法仅在调用 `ITransaction.Execute` 方法时执行。</span><span class="sxs-lookup"><span data-stu-id="28e15-531">This means that they are only performed when the `ITransaction.Execute` method is invoked.</span></span> <span data-ttu-id="28e15-532">`ITransaction.Execute` 方法返回的值指示事务创建是成功 (true) 还是失败 (false)。</span><span class="sxs-lookup"><span data-stu-id="28e15-532">The value that's returned by the `ITransaction.Execute` method indicates whether the transaction was created successfully (true) or if it failed (false).</span></span>

<span data-ttu-id="28e15-533">以下代码段显示的示例会在执行同一事务期间递增和递减两个计数器：</span><span class="sxs-lookup"><span data-stu-id="28e15-533">The following code snippet shows an example that increments and decrements two counters as part of the same transaction:</span></span>
...
<span data-ttu-id="28e15-534">请记住，Redis 事务不同于关系数据库中的事务。</span><span class="sxs-lookup"><span data-stu-id="28e15-534">Remember that Redis transactions are unlike transactions in relational databases.</span></span> <span data-ttu-id="28e15-535">`Execute` 方法只是将构成运行事务的所有命令排入队列，如果其中任何一个命令格式不当，则停止事务。</span><span class="sxs-lookup"><span data-stu-id="28e15-535">The `Execute` method simply queues all the commands that comprise the transaction to be run, and if any of them is malformed then the transaction is stopped.</span></span> <span data-ttu-id="28e15-536">如果已成功将所有命令排入队列，以异步方式运行每个命令。</span><span class="sxs-lookup"><span data-stu-id="28e15-536">If all the commands have been queued successfully, each command runs asynchronously.</span></span>

<span data-ttu-id="28e15-537">如果任何命令失败，其他命令仍将继续处理。</span><span class="sxs-lookup"><span data-stu-id="28e15-537">If any command fails, the others still continue processing.</span></span> <span data-ttu-id="28e15-538">如果需要验证命令是否已成功完成，必须使用相应任务的 **Result** 属性来提取命令的结果，如上述示例中所示。</span><span class="sxs-lookup"><span data-stu-id="28e15-538">If you need to verify that a command has completed successfully, you must fetch the results of the command by using the **Result** property of the corresponding task, as shown in the example above.</span></span> <span data-ttu-id="28e15-539">读取 **Result** 属性会阻塞调用线程，直到任务完成。</span><span class="sxs-lookup"><span data-stu-id="28e15-539">Reading the **Result** property will block the calling thread until the task has completed.</span></span>

<span data-ttu-id="28e15-540">有关详细信息，请参阅 [Transactions in Redis](https://stackexchange.github.io/StackExchange.Redis/Transactions)（Redis 中的事务）。</span><span class="sxs-lookup"><span data-stu-id="28e15-540">For more information, see [Transactions in Redis](https://stackexchange.github.io/StackExchange.Redis/Transactions).</span></span>

<span data-ttu-id="28e15-541">执行批处理操作时，可以使用 StackExchange 库的 `IBatch` 接口。</span><span class="sxs-lookup"><span data-stu-id="28e15-541">When performing batch operations, you can use the `IBatch` interface of the StackExchange library.</span></span> <span data-ttu-id="28e15-542">此接口可用于访问 `IDatabase` 接口所访问的类似一组方法，不过，所有方法是异步的。</span><span class="sxs-lookup"><span data-stu-id="28e15-542">This interface provides access to a set of methods similar to those accessed by the `IDatabase` interface, except that all the methods are asynchronous.</span></span>

<span data-ttu-id="28e15-543">可以使用 `IDatabase.CreateBatch` 方法来创建 `IBatch` 对象，并使用 `IBatch.Execute` 方法来运行批处理，如以下示例所示。</span><span class="sxs-lookup"><span data-stu-id="28e15-543">You create an `IBatch` object by using the `IDatabase.CreateBatch` method, and then run the batch by using the `IBatch.Execute` method, as shown in the following example.</span></span> <span data-ttu-id="28e15-544">这段代码仅设置字符串值，递增和递减前面示例中使用的相同计数器，并显示结果：</span><span class="sxs-lookup"><span data-stu-id="28e15-544">This code simply sets a string value, increments and decrements the same counters used in the previous example, and displays the results:</span></span>
...
<span data-ttu-id="28e15-545">必须知道，这不同于事务，如果因为格式不当而导致批中的命令失败，其他命令仍可运行。</span><span class="sxs-lookup"><span data-stu-id="28e15-545">It is important to understand that unlike a transaction, if a command in a batch fails because it is malformed, the other commands might still run.</span></span> <span data-ttu-id="28e15-546">`IBatch.Execute` 方法不返回成功或失败的任何指示。</span><span class="sxs-lookup"><span data-stu-id="28e15-546">The `IBatch.Execute` method does not return any indication of success or failure.</span></span>

### <a name="perform-fire-and-forget-cache-operations"></a><span data-ttu-id="28e15-547">执行即发即弃缓存操作</span><span class="sxs-lookup"><span data-stu-id="28e15-547">Perform fire and forget cache operations</span></span>

<span data-ttu-id="28e15-548">Redis 通过使用命令标志来支持即发即弃操作。</span><span class="sxs-lookup"><span data-stu-id="28e15-548">Redis supports fire and forget operations by using command flags.</span></span> <span data-ttu-id="28e15-549">在此情况下，客户端仅启动操作，但不关注结果，且并不会等待命令完成。</span><span class="sxs-lookup"><span data-stu-id="28e15-549">In this situation, the client simply initiates an operation but has no interest in the result and does not wait for the command to be completed.</span></span> <span data-ttu-id="28e15-550">以下示例演示了如何以即发即弃操作的形式执行 INCR 命令：</span><span class="sxs-lookup"><span data-stu-id="28e15-550">The example below shows how to perform the INCR command as a fire and forget operation:</span></span>
...
### <a name="specify-automatically-expiring-keys"></a><span data-ttu-id="28e15-551">指定密钥自动过期</span><span class="sxs-lookup"><span data-stu-id="28e15-551">Specify automatically expiring keys</span></span>

<span data-ttu-id="28e15-552">在 Redis 缓存中存储项时，可以指定超时，超时过后，会自动从缓存中删除该项。</span><span class="sxs-lookup"><span data-stu-id="28e15-552">When you store an item in a Redis cache, you can specify a timeout after which the item will be automatically removed from the cache.</span></span> <span data-ttu-id="28e15-553">还可以在密钥过期之前，使用 `TTL` 命令来查询剩余时间。</span><span class="sxs-lookup"><span data-stu-id="28e15-553">You can also query how much more time a key has before it expires by using the `TTL` command.</span></span> <span data-ttu-id="28e15-554">StackExchange 应用程序可通过 `IDatabase.KeyTimeToLive` 方法使用此命令。</span><span class="sxs-lookup"><span data-stu-id="28e15-554">This command is available to StackExchange applications by using the `IDatabase.KeyTimeToLive` method.</span></span>

<span data-ttu-id="28e15-555">以下代码段演示了如何将密钥过期时间设置为 20 秒，并查询密钥剩余生存期：</span><span class="sxs-lookup"><span data-stu-id="28e15-555">The following code snippet shows how to set an expiration time of 20 seconds on a key, and query the remaining lifetime of the key:</span></span>
...
<span data-ttu-id="28e15-556">还可以使用 StackExchange 库中作为 `KeyExpireAsync` 方法提供的 EXPIRE 命令将过期时间设置为特定的日期和时间：</span><span class="sxs-lookup"><span data-stu-id="28e15-556">You can also set the expiration time to a specific date and time by using the EXPIRE command, which is available in the StackExchange library as the `KeyExpireAsync` method:</span></span>
...
> [!TIP]
> <span data-ttu-id="28e15-557">可以使用 DEL 命令手动从缓存中删除项，该命令在 StackExchange 库中作为 `IDatabase.KeyDeleteAsync` 方法提供。</span><span class="sxs-lookup"><span data-stu-id="28e15-557">You can manually remove an item from the cache by using the DEL command, which is available through the StackExchange library as the `IDatabase.KeyDeleteAsync` method.</span></span>

### <a name="use-tags-to-cross-correlate-cached-items"></a><span data-ttu-id="28e15-558">使用标记来交叉关联项</span><span class="sxs-lookup"><span data-stu-id="28e15-558">Use tags to cross-correlate cached items</span></span>

<span data-ttu-id="28e15-559">Redis 集是共享单个键的多个项集合。</span><span class="sxs-lookup"><span data-stu-id="28e15-559">A Redis set is a collection of multiple items that share a single key.</span></span> <span data-ttu-id="28e15-560">可以使用 SADD 命令来创建集。</span><span class="sxs-lookup"><span data-stu-id="28e15-560">You can create a set by using the SADD command.</span></span> <span data-ttu-id="28e15-561">可以使用 SMEMBERS 命令来检索集中的项。</span><span class="sxs-lookup"><span data-stu-id="28e15-561">You can retrieve the items in a set by using the SMEMBERS command.</span></span> <span data-ttu-id="28e15-562">StackExchange 库通过 `IDatabase.SetAddAsync` 方法实现 SADD 命令，并使用 `IDatabase.SetMembersAsync` 方法实现 SMEMBERS 命令。</span><span class="sxs-lookup"><span data-stu-id="28e15-562">The StackExchange library implements the SADD command with the `IDatabase.SetAddAsync` method, and the SMEMBERS command with the `IDatabase.SetMembersAsync` method.</span></span>

<span data-ttu-id="28e15-563">还可以使用 SDIFF（差集）、SINTER（交集）和 SUNION（并集）命令来合并现有集以创建新的集。</span><span class="sxs-lookup"><span data-stu-id="28e15-563">You can also combine existing sets to create new sets by using the SDIFF (set difference), SINTER (set intersection), and SUNION (set union) commands.</span></span> <span data-ttu-id="28e15-564">StackExchange 库在 `IDatabase.SetCombineAsync` 方法中统一了这些操作。</span><span class="sxs-lookup"><span data-stu-id="28e15-564">The StackExchange library unifies these operations in the `IDatabase.SetCombineAsync` method.</span></span> <span data-ttu-id="28e15-565">此方法的第一个参数指定要执行的设置操作。</span><span class="sxs-lookup"><span data-stu-id="28e15-565">The first parameter to this method specifies the set operation to perform.</span></span>

<span data-ttu-id="28e15-566">以下代码段演示了如何使用集来快速存储和检索相关项的集合。</span><span class="sxs-lookup"><span data-stu-id="28e15-566">The following code snippets show how sets can be useful for quickly storing and retrieving collections of related items.</span></span> <span data-ttu-id="28e15-567">此代码使用本文前面的“实施 Redis 缓存客户端应用程序”部分中所述的 `BlogPost` 类型。</span><span class="sxs-lookup"><span data-stu-id="28e15-567">This code uses the `BlogPost` type that was described in the section Implement Redis Cache Client Applications earlier in this article.</span></span>

<span data-ttu-id="28e15-568">`BlogPost` 对象包含四个字段：ID、标题、排名分数和标记集合。</span><span class="sxs-lookup"><span data-stu-id="28e15-568">A `BlogPost` object contains four fields—an ID, a title, a ranking score, and a collection of tags.</span></span> <span data-ttu-id="28e15-569">以下第一个代码片段演示了用于填充 `BlogPost` 对象的 C# 列表的示例数据：</span><span class="sxs-lookup"><span data-stu-id="28e15-569">The first code snippet below shows the sample data that's used for populating a C# list of `BlogPost` objects:</span></span>
...
<span data-ttu-id="28e15-570">可以在 Redis 缓存中针对每个 `BlogPost` 对象将标记存储为集，并将每个集与 `BlogPost` ID关联。</span><span class="sxs-lookup"><span data-stu-id="28e15-570">You can store the tags for each `BlogPost` object as a set in a Redis cache and associate each set with the ID of the `BlogPost`.</span></span> <span data-ttu-id="28e15-571">这样，应用程序便可以快速查找属于特定博客文章的所有标记。</span><span class="sxs-lookup"><span data-stu-id="28e15-571">This enables an application to quickly find all the tags that belong to a specific blog post.</span></span> <span data-ttu-id="28e15-572">若要启用反向搜索并查找所有共享特定标记的博客文章，可以创建另一个集，用于保存引用键中标记 ID 的博客文章：</span><span class="sxs-lookup"><span data-stu-id="28e15-572">To enable searching in the opposite direction and find all blog posts that share a specific tag, you can create another set that holds the blog posts referencing the tag ID in the key:</span></span>
...
<span data-ttu-id="28e15-573">这些结构允许高效执行许多常见查询。</span><span class="sxs-lookup"><span data-stu-id="28e15-573">These structures enable you to perform many common queries very efficiently.</span></span> <span data-ttu-id="28e15-574">例如，可以按如下所示查找并显示博客文章 1 的所有标记：</span><span class="sxs-lookup"><span data-stu-id="28e15-574">For example, you can find and display all of the tags for blog post 1 like this:</span></span>
...
<span data-ttu-id="28e15-575">可以通过执行交集操作，查找博客文章 1 和博客文章 2 公用的所有标记，如下所示：</span><span class="sxs-lookup"><span data-stu-id="28e15-575">You can find all tags that are common to blog post 1 and blog post 2 by performing a set intersection operation, as follows:</span></span>
...
<span data-ttu-id="28e15-576">可以查找包含特定标记的所有博客文章：</span><span class="sxs-lookup"><span data-stu-id="28e15-576">And you can find all blog posts that contain a specific tag:</span></span>
...
### <a name="find-recently-accessed-items"></a><span data-ttu-id="28e15-577">查找最近访问的项</span><span class="sxs-lookup"><span data-stu-id="28e15-577">Find recently accessed items</span></span>

<span data-ttu-id="28e15-578">许多应用程序需要执行的常见任务是查找最近访问的项。</span><span class="sxs-lookup"><span data-stu-id="28e15-578">A common task required of many applications is to find the most recently accessed items.</span></span> <span data-ttu-id="28e15-579">例如，博客站点可能要显示有关最近读过的博客文章的信息。</span><span class="sxs-lookup"><span data-stu-id="28e15-579">For example, a blogging site might want to display information about the most recently read blog posts.</span></span>

<span data-ttu-id="28e15-580">可以使用 Redis 列表来实现此功能。</span><span class="sxs-lookup"><span data-stu-id="28e15-580">You can implement this functionality by using a Redis list.</span></span> <span data-ttu-id="28e15-581">Redis 列表包含共享同一个键的多个项。</span><span class="sxs-lookup"><span data-stu-id="28e15-581">A Redis list contains multiple items that share the same key.</span></span> <span data-ttu-id="28e15-582">该列表充当双端队列。</span><span class="sxs-lookup"><span data-stu-id="28e15-582">The list acts as a double-ended queue.</span></span> <span data-ttu-id="28e15-583">可以使用 LPUSH（左推）和 RPUSH（右推）命令将项推送到列表一端。</span><span class="sxs-lookup"><span data-stu-id="28e15-583">You can push items to either end of the list by using the LPUSH (left push) and RPUSH (right push) commands.</span></span> <span data-ttu-id="28e15-584">可以使用 LPOP 和 RPOP 命令从列表的一端检索项。</span><span class="sxs-lookup"><span data-stu-id="28e15-584">You can retrieve items from either end of the list by using the LPOP and RPOP commands.</span></span> <span data-ttu-id="28e15-585">还可以使用 LRANGE 和 RRANGE 命令返回一组元素。</span><span class="sxs-lookup"><span data-stu-id="28e15-585">You can also return a set of elements by using the LRANGE and RRANGE commands.</span></span>

<span data-ttu-id="28e15-586">以下代码段演示了如何使用 StackExchange 库来执行这些操作。</span><span class="sxs-lookup"><span data-stu-id="28e15-586">The code snippets below show how you can perform these operations by using the StackExchange library.</span></span> <span data-ttu-id="28e15-587">此代码使用前面示例中的 `BlogPost` 类型。</span><span class="sxs-lookup"><span data-stu-id="28e15-587">This code uses the `BlogPost` type from the previous examples.</span></span> <span data-ttu-id="28e15-588">当用户阅读博客文章时，`IDatabase.ListLeftPushAsync` 方法将博客文章的标题推送到与 Redis 缓存中的“blog:recent_posts”键关联的列表。</span><span class="sxs-lookup"><span data-stu-id="28e15-588">As a blog post is read by a user, the `IDatabase.ListLeftPushAsync` method pushes the title of the blog post onto a list that's associated with the key "blog:recent_posts" in the Redis cache.</span></span>
...
<span data-ttu-id="28e15-589">随着阅读的博客文章越来越多，其标题将推送到同一列表。</span><span class="sxs-lookup"><span data-stu-id="28e15-589">As more blog posts are read, their titles are pushed onto the same list.</span></span> <span data-ttu-id="28e15-590">列表已根据其添加顺序进行排序。</span><span class="sxs-lookup"><span data-stu-id="28e15-590">The list is ordered by the sequence in which the titles have been added.</span></span> <span data-ttu-id="28e15-591">最近阅读的博客文章朝向列表左端。</span><span class="sxs-lookup"><span data-stu-id="28e15-591">The most recently read blog posts are towards the left end of the list.</span></span> <span data-ttu-id="28e15-592">（如果同一博客文章阅读了一次以上，则它在列表中有多个条目。）</span><span class="sxs-lookup"><span data-stu-id="28e15-592">(If the same blog post is read more than once, it will have multiple entries in the list.)</span></span>

<span data-ttu-id="28e15-593">可以使用 `IDatabase.ListRange` 方法显示最近阅读的文章的标题。</span><span class="sxs-lookup"><span data-stu-id="28e15-593">You can display the titles of the most recently read posts by using the `IDatabase.ListRange` method.</span></span> <span data-ttu-id="28e15-594">此方法采用包含列表、起点和终点的键。</span><span class="sxs-lookup"><span data-stu-id="28e15-594">This method takes the key that contains the list, a starting point, and an ending point.</span></span> <span data-ttu-id="28e15-595">以下代码将从列表的最左端检索 10 篇博客文章的标题（项为 0 到 9）：</span><span class="sxs-lookup"><span data-stu-id="28e15-595">The following code retrieves the titles of the 10 blog posts (items from 0 to 9) at the left-most end of the list:</span></span>
...
<span data-ttu-id="28e15-596">请注意，`ListRangeAsync` 方法不会从列表中删除项。</span><span class="sxs-lookup"><span data-stu-id="28e15-596">Note that the `ListRangeAsync` method does not remove items from the list.</span></span> <span data-ttu-id="28e15-597">为此，可以使用 `IDatabase.ListLeftPopAsync` 和 `IDatabase.ListRightPopAsync` 方法。</span><span class="sxs-lookup"><span data-stu-id="28e15-597">To do this, you can use the `IDatabase.ListLeftPopAsync` and `IDatabase.ListRightPopAsync` methods.</span></span>

<span data-ttu-id="28e15-598">若要防止列表无限增长，可以通过修剪列表来定期删除项。</span><span class="sxs-lookup"><span data-stu-id="28e15-598">To prevent the list from growing indefinitely, you can periodically cull items by trimming the list.</span></span> <span data-ttu-id="28e15-599">以下代码段演示了如何只保留列表中位于最左端的 5 个项并删除其他所有项：</span><span class="sxs-lookup"><span data-stu-id="28e15-599">The code snippet below shows you how to remove all but the five left-most items from the list:</span></span>

```csharp
await cache.ListTrimAsync(redisKey, 0, 5);
```

### <a name="implement-a-leader-board"></a><span data-ttu-id="28e15-600">实施排行榜</span><span class="sxs-lookup"><span data-stu-id="28e15-600">Implement a leader board</span></span>

<span data-ttu-id="28e15-601">默认情况下，集中的项不以任何特定顺序保存。</span><span class="sxs-lookup"><span data-stu-id="28e15-601">By default, the items in a set are not held in any specific order.</span></span> <span data-ttu-id="28e15-602">可以使用 ZADD 命令（StackExchange 库中的 `IDatabase.SortedSetAdd` 方法）来创建排序集合。</span><span class="sxs-lookup"><span data-stu-id="28e15-602">You can create an ordered set by using the ZADD command (the `IDatabase.SortedSetAdd` method in the StackExchange library).</span></span> <span data-ttu-id="28e15-603">系统使用一个名为 score（作为命令的参数提供）的数字值来为项排序。</span><span class="sxs-lookup"><span data-stu-id="28e15-603">The items are ordered by using a numeric value called a score, which is provided as a parameter to the command.</span></span>

<span data-ttu-id="28e15-604">以下代码段将博客文章的标题添加到排序列表。</span><span class="sxs-lookup"><span data-stu-id="28e15-604">The following code snippet adds the title of a blog post to an ordered list.</span></span> <span data-ttu-id="28e15-605">在示例中，每篇博客文章还有包含博客文章排名的评分字段。</span><span class="sxs-lookup"><span data-stu-id="28e15-605">In this example, each blog post also has a score field that contains the ranking of the blog post.</span></span>
...
<span data-ttu-id="28e15-606">可以使用 `IDatabase.SortedSetRangeByRankWithScores` 方法以评分递增顺序来检索博客文章标题和评分：</span><span class="sxs-lookup"><span data-stu-id="28e15-606">You can retrieve the blog post titles and scores in ascending score order by using the `IDatabase.SortedSetRangeByRankWithScores` method:</span></span>
...
> [!NOTE]
> <span data-ttu-id="28e15-607">StackExchange 库还提供了 `IDatabase.SortedSetRangeByRankAsync` 方法，用于以评分顺序返回数据，但不返回评分。</span><span class="sxs-lookup"><span data-stu-id="28e15-607">The StackExchange library also provides the `IDatabase.SortedSetRangeByRankAsync` method, which returns the data in score order, but does not return the scores.</span></span>

<span data-ttu-id="28e15-608">也可以使用评分递减顺序来检索项，并通过将额外参数提供给 `IDatabase.SortedSetRangeByRankWithScoresAsync` 方法来限制返回项的数目。</span><span class="sxs-lookup"><span data-stu-id="28e15-608">You can also retrieve items in descending order of scores, and limit the number of items that are returned by providing additional parameters to the `IDatabase.SortedSetRangeByRankWithScoresAsync` method.</span></span> <span data-ttu-id="28e15-609">以下示例演示了排名前 10 位博客文章的标题和评分：</span><span class="sxs-lookup"><span data-stu-id="28e15-609">The next example displays the titles and scores of the top 10 ranked blog posts:</span></span>
...
<span data-ttu-id="28e15-610">以下示例使用了 `IDatabase.SortedSetRangeByScoreWithScoresAsync` 方法，该方法可用于限制返回给那些处于给定评分范围内的项：</span><span class="sxs-lookup"><span data-stu-id="28e15-610">The next example uses the `IDatabase.SortedSetRangeByScoreWithScoresAsync` method, which you can use to limit the items that are returned to those that fall within a given score range:</span></span>
...
### <a name="message-by-using-channels"></a><span data-ttu-id="28e15-611">使用通道进行消息传送</span><span class="sxs-lookup"><span data-stu-id="28e15-611">Message by using channels</span></span>

<span data-ttu-id="28e15-612">Redis 服务器除了可用作数据缓存以外，还可通过高性能发布者/订阅者机制提供消息传送。</span><span class="sxs-lookup"><span data-stu-id="28e15-612">Apart from acting as a data cache, a Redis server provides messaging through a high-performance publisher/subscriber mechanism.</span></span> <span data-ttu-id="28e15-613">客户端应用程序可以订阅通道，其他应用程序或服务可以将消息发布到通道。</span><span class="sxs-lookup"><span data-stu-id="28e15-613">Client applications can subscribe to a channel, and other applications or services can publish messages to the channel.</span></span> <span data-ttu-id="28e15-614">订阅应用程序随后会接收这些消息，并可以处理消息。</span><span class="sxs-lookup"><span data-stu-id="28e15-614">Subscribing applications will then receive these messages and can process them.</span></span>

<span data-ttu-id="28e15-615">Redis 提供 SUBSCRIBE 命令来让客户端应用程序订阅通道。</span><span class="sxs-lookup"><span data-stu-id="28e15-615">Redis provides the SUBSCRIBE command for client applications to use to subscribe to channels.</span></span> <span data-ttu-id="28e15-616">此命令需要一个或多个可供应用程序接受消息的通道的名称。</span><span class="sxs-lookup"><span data-stu-id="28e15-616">This command expects the name of one or more channels on which the application will accept messages.</span></span> <span data-ttu-id="28e15-617">StackExchange 库包含 `ISubscription` 接口，可让 .NET Framework 应用程序订阅和发布到通道。</span><span class="sxs-lookup"><span data-stu-id="28e15-617">The StackExchange library includes the `ISubscription` interface, which enables a .NET Framework application to subscribe and publish to channels.</span></span>

<span data-ttu-id="28e15-618">使用 Redis 服务器连接的 `GetSubscriber` 方法创建 `ISubscription` 对象。</span><span class="sxs-lookup"><span data-stu-id="28e15-618">You create an `ISubscription` object by using the `GetSubscriber` method of the connection to the Redis server.</span></span> <span data-ttu-id="28e15-619">然后使用此对象的 `SubscribeAsync` 方法在通道上侦听消息。</span><span class="sxs-lookup"><span data-stu-id="28e15-619">Then you listen for messages on a channel by using the `SubscribeAsync` method of this object.</span></span> <span data-ttu-id="28e15-620">以下代码示例演示了如何订阅名为“messages:blogPosts”的通道：</span><span class="sxs-lookup"><span data-stu-id="28e15-620">The following code example shows how to subscribe to a channel named "messages:blogPosts":</span></span>
...
<span data-ttu-id="28e15-621">`Subscribe` 方法的第一个参数为通道的名称。</span><span class="sxs-lookup"><span data-stu-id="28e15-621">The first parameter to the `Subscribe` method is the name of the channel.</span></span> <span data-ttu-id="28e15-622">此名称遵循缓存中键使用的相同约定。</span><span class="sxs-lookup"><span data-stu-id="28e15-622">This name follows the same conventions that are used by keys in the cache.</span></span> <span data-ttu-id="28e15-623">该名称可以包含任何二进制数据，但建议最好使用相对较短且有意义的字符串，以帮助确保良好的性能和易维护性。</span><span class="sxs-lookup"><span data-stu-id="28e15-623">The name can contain any binary data, although it is advisable to use relatively short, meaningful strings to help ensure good performance and maintainability.</span></span>

<span data-ttu-id="28e15-624">另请注意，通道使用的命名空间与键使用的不同。</span><span class="sxs-lookup"><span data-stu-id="28e15-624">Note also that the namespace used by channels is separate from that used by keys.</span></span> <span data-ttu-id="28e15-625">这意味着，通道和键可以同名，不过，这可能会导致更难以维护应用程序代码。</span><span class="sxs-lookup"><span data-stu-id="28e15-625">This means you can have channels and keys that have the same name, although this may make your application code more difficult to maintain.</span></span>

<span data-ttu-id="28e15-626">第二个参数是 Action 委派。</span><span class="sxs-lookup"><span data-stu-id="28e15-626">The second parameter is an Action delegate.</span></span> <span data-ttu-id="28e15-627">每当新的消息出现在通道上时，此委派就会以异步方式运行。</span><span class="sxs-lookup"><span data-stu-id="28e15-627">This delegate runs asynchronously whenever a new message appears on the channel.</span></span> <span data-ttu-id="28e15-628">此示例仅显示了控制台上的消息（消息将包含博客文章的标题）。</span><span class="sxs-lookup"><span data-stu-id="28e15-628">This example simply displays the message on the console (the message will contain the title of a blog post).</span></span>

<span data-ttu-id="28e15-629">若要发布到通道，应用程序可以使用 Redis PUBLISH 命令。</span><span class="sxs-lookup"><span data-stu-id="28e15-629">To publish to a channel, an application can use the Redis PUBLISH command.</span></span> <span data-ttu-id="28e15-630">StackExchange 库提供了 `IServer.PublishAsync` 方法来执行此操作。</span><span class="sxs-lookup"><span data-stu-id="28e15-630">The StackExchange library provides the `IServer.PublishAsync` method to perform this operation.</span></span> <span data-ttu-id="28e15-631">以下代码片段演示了如何将消息发布到“messages:blogPosts”通道：</span><span class="sxs-lookup"><span data-stu-id="28e15-631">The next code snippet shows how to publish a message to the "messages:blogPosts" channel:</span></span>
...
<span data-ttu-id="28e15-632">关于发布/订阅机制，应该了解几个要点：</span><span class="sxs-lookup"><span data-stu-id="28e15-632">There are several points you should understand about the publish/subscribe mechanism:</span></span>

- <span data-ttu-id="28e15-633">多个订阅者可以订阅同一个通道，他们都将接收发布到该通道的消息。</span><span class="sxs-lookup"><span data-stu-id="28e15-633">Multiple subscribers can subscribe to the same channel, and they will all receive the messages that are published to that channel.</span></span>
- <span data-ttu-id="28e15-634">订阅者仅接收订阅后发布的消息。</span><span class="sxs-lookup"><span data-stu-id="28e15-634">Subscribers only receive messages that have been published after they have subscribed.</span></span> <span data-ttu-id="28e15-635">通道不会缓冲，一旦发布消息，Redis 基础结构就会将消息推送到每个订阅者，然后删除消息。</span><span class="sxs-lookup"><span data-stu-id="28e15-635">Channels are not buffered, and once a message is published, the Redis infrastructure pushes the message to each subscriber and then removes it.</span></span>
- <span data-ttu-id="28e15-636">默认情况下，订阅者根据发送顺序来接收消息。</span><span class="sxs-lookup"><span data-stu-id="28e15-636">By default, messages are received by subscribers in the order in which they are sent.</span></span> <span data-ttu-id="28e15-637">在具有大量消息和许多订阅者与发布者的高度活跃系统中，保证依序传送消息可能会降低系统性能。</span><span class="sxs-lookup"><span data-stu-id="28e15-637">In a highly active system with a large number of messages and many subscribers and publishers, guaranteed sequential delivery of messages can slow performance of the system.</span></span> <span data-ttu-id="28e15-638">如果每个消息各自独立且顺序并不重要，则可以通过 Redis 系统启用并发处理，这有助于提高响应度。</span><span class="sxs-lookup"><span data-stu-id="28e15-638">If each message is independent and the order is unimportant, you can enable concurrent processing by the Redis system, which can help to improve responsiveness.</span></span> <span data-ttu-id="28e15-639">可以在 StackExchange 客户端中，通过将订阅者使用的连接的 PreserveAsyncOrder 设置为 false 来实现此目的：</span><span class="sxs-lookup"><span data-stu-id="28e15-639">You can achieve this in a StackExchange client by setting the PreserveAsyncOrder of the connection used by the subscriber to false:</span></span>
...
### <a name="serialization-considerations"></a><span data-ttu-id="28e15-640">序列化注意事项</span><span class="sxs-lookup"><span data-stu-id="28e15-640">Serialization considerations</span></span>

<span data-ttu-id="28e15-641">选择序列化格式时，请考虑在性能、互操作性、版本控制、与现有系统的兼容性、数据压缩和内存开销之间作出权衡。</span><span class="sxs-lookup"><span data-stu-id="28e15-641">When you choose a serialization format, consider tradeoffs between performance, interoperability, versioning, compatibility with existing systems, data compression, and memory overhead.</span></span> <span data-ttu-id="28e15-642">评估性能时，请记住基准高度依赖于上下文。</span><span class="sxs-lookup"><span data-stu-id="28e15-642">When you are evaluating performance, remember that benchmarks are highly dependent on context.</span></span> <span data-ttu-id="28e15-643">它们可能不反映实际工作负载，也可能不考虑较新的库或版本。</span><span class="sxs-lookup"><span data-stu-id="28e15-643">They may not reflect your actual workload, and may not consider newer libraries or versions.</span></span> <span data-ttu-id="28e15-644">不存在适用于所有方案的“最快”序列化程序。</span><span class="sxs-lookup"><span data-stu-id="28e15-644">There is no single "fastest" serializer for all scenarios.</span></span>

<span data-ttu-id="28e15-645">应考虑的一些选项包括：</span><span class="sxs-lookup"><span data-stu-id="28e15-645">Some options to consider include:</span></span>

- <span data-ttu-id="28e15-646">[Protocol Buffers](https://github.com/google/protobuf)（也称为 protobuf）是由 Google 开发的序列化格式，用于对结构化数据进行高效序列化。</span><span class="sxs-lookup"><span data-stu-id="28e15-646">[Protocol Buffers](https://github.com/google/protobuf) (also called protobuf) is a serialization format developed by Google for serializing structured data efficiently.</span></span> <span data-ttu-id="28e15-647">它使用强类型定义文件定义消息结构。</span><span class="sxs-lookup"><span data-stu-id="28e15-647">It uses strongly-typed definition files to define message structures.</span></span> <span data-ttu-id="28e15-648">然后，将这些定义文件编译为用于对消息进行序列化和反序列化的特定语言代码。</span><span class="sxs-lookup"><span data-stu-id="28e15-648">These definition files are then compiled to language-specific code for serializing and deserializing messages.</span></span> <span data-ttu-id="28e15-649">Protobuf 可用于现有的 RPC 机制，还可以生成 RPC 服务。</span><span class="sxs-lookup"><span data-stu-id="28e15-649">Protobuf can be used over existing RPC mechanisms, or it can generate an RPC service.</span></span>

- <span data-ttu-id="28e15-650">[Apache Thrift](https://thrift.apache.org/) 使用类似的方法，即使用强类型定义文件和编译步骤生成序列化代码和 RPC 服务。</span><span class="sxs-lookup"><span data-stu-id="28e15-650">[Apache Thrift](https://thrift.apache.org/) uses a similar approach, with strongly typed definition files and a compilation step to generate the serialization code and RPC services.</span></span>

- <span data-ttu-id="28e15-651">[Apache Avro](https://avro.apache.org/) 向 Protocol Buffers 和 Thrift 提供类似的功能，但不提供编译步骤。</span><span class="sxs-lookup"><span data-stu-id="28e15-651">[Apache Avro](https://avro.apache.org/) provides similar functionality to Protocol Buffers and Thrift, but there is no compilation step.</span></span> <span data-ttu-id="28e15-652">相反，序列化数据始终包含描述结构的架构。</span><span class="sxs-lookup"><span data-stu-id="28e15-652">Instead, serialized data always includes a schema that describes the structure.</span></span>

- <span data-ttu-id="28e15-653">[JSON](https://json.org/) 是使用可人工读取的文本字段的开放标准。</span><span class="sxs-lookup"><span data-stu-id="28e15-653">[JSON](https://json.org/) is an open standard that uses human-readable text fields.</span></span> <span data-ttu-id="28e15-654">它提供广泛的跨平台支持。</span><span class="sxs-lookup"><span data-stu-id="28e15-654">It has broad cross-platform support.</span></span> <span data-ttu-id="28e15-655">JSON 不使用消息架构。</span><span class="sxs-lookup"><span data-stu-id="28e15-655">JSON does not use message schemas.</span></span> <span data-ttu-id="28e15-656">它是基于文本的格式，在网络上的效率不高。</span><span class="sxs-lookup"><span data-stu-id="28e15-656">Being a text-based format, it is not very efficient over the wire.</span></span> <span data-ttu-id="28e15-657">但是，在某些情况下，可通过 HTTP 直接向客户端返回缓存项，此时，存储 JSON 可以节省从另一格式反序列化然后再序列化为 JSON 的成本。</span><span class="sxs-lookup"><span data-stu-id="28e15-657">In some cases, however, you may be returning cached items directly to a client via HTTP, in which case storing JSON could save the cost of deserializing from another format and then serializing to JSON.</span></span>

- <span data-ttu-id="28e15-658">[BSON](http://bsonspec.org/) 是一种二进制序列化格式，使用的结构与 JSON 类似。</span><span class="sxs-lookup"><span data-stu-id="28e15-658">[BSON](http://bsonspec.org/) is a binary serialization format that uses a structure similar to JSON.</span></span> <span data-ttu-id="28e15-659">相对于 JSON，BSON 旨在轻量、易于扫描以及可快速进行序列化和反序列化。</span><span class="sxs-lookup"><span data-stu-id="28e15-659">BSON was designed to be lightweight, easy to scan, and fast to serialize and deserialize, relative to JSON.</span></span> <span data-ttu-id="28e15-660">其有效负载的大小与 JSON 相当。</span><span class="sxs-lookup"><span data-stu-id="28e15-660">Payloads are comparable in size to JSON.</span></span> <span data-ttu-id="28e15-661">BSON 有效负载可能大于或小于 JSON 有效负载，具体取决于数据。</span><span class="sxs-lookup"><span data-stu-id="28e15-661">Depending on the data, a BSON payload may be smaller or larger than a JSON payload.</span></span> <span data-ttu-id="28e15-662">BSON 拥有 JSON 中不支持的一些其他数据类型，尤其是 BinData（用于字节数组）和日期。</span><span class="sxs-lookup"><span data-stu-id="28e15-662">BSON has some additional data types that are not available in JSON, notably BinData (for byte arrays) and Date.</span></span>

- <span data-ttu-id="28e15-663">[MessagePack](https://msgpack.org/) 是旨在压缩网络传输的二进制序列化格式。</span><span class="sxs-lookup"><span data-stu-id="28e15-663">[MessagePack](https://msgpack.org/) is a binary serialization format that is designed to be compact for transmission over the wire.</span></span> <span data-ttu-id="28e15-664">没有消息架构或消息类型检查。</span><span class="sxs-lookup"><span data-stu-id="28e15-664">There are no message schemas or message type checking.</span></span>

- <span data-ttu-id="28e15-665">[Bond](https://microsoft.github.io/bond/) 是用于系统化数据的跨平台框架。</span><span class="sxs-lookup"><span data-stu-id="28e15-665">[Bond](https://microsoft.github.io/bond/) is a cross-platform framework for working with schematized data.</span></span> <span data-ttu-id="28e15-666">它支持跨语言序列化和反序列化。</span><span class="sxs-lookup"><span data-stu-id="28e15-666">It supports cross-language serialization and deserialization.</span></span> <span data-ttu-id="28e15-667">此处列出的与其他系统的显著差异包括对继承、类型别名和泛型的支持。</span><span class="sxs-lookup"><span data-stu-id="28e15-667">Notable differences from other systems listed here are support for inheritance, type aliases, and generics.</span></span>

- <span data-ttu-id="28e15-668">[gRPC](https://www.grpc.io/) 是由 Google 开发的开放源 RPC 系统。</span><span class="sxs-lookup"><span data-stu-id="28e15-668">[gRPC](https://www.grpc.io/) is an open source RPC system developed by Google.</span></span> <span data-ttu-id="28e15-669">默认情况下，它将 Protocol Buffers 用作其定义语言和基础消息交换格式。</span><span class="sxs-lookup"><span data-stu-id="28e15-669">By default, it uses Protocol Buffers as its definition language and underlying message interchange format.</span></span>

## <a name="related-patterns-and-guidance"></a><span data-ttu-id="28e15-670">相关模式和指南</span><span class="sxs-lookup"><span data-stu-id="28e15-670">Related patterns and guidance</span></span>

<span data-ttu-id="28e15-671">在应用程序中实施缓存时，以下模式也可能与方案相关：</span><span class="sxs-lookup"><span data-stu-id="28e15-671">The following patterns might also be relevant to your scenario when you implement caching in your applications:</span></span>

- <span data-ttu-id="28e15-672">[缓存端模式](../patterns/cache-aside.md)：此模式描述如何按需将数据从数据存储载入缓存。</span><span class="sxs-lookup"><span data-stu-id="28e15-672">[Cache-aside pattern](../patterns/cache-aside.md): This pattern describes how to load data on demand into a cache from a data store.</span></span> <span data-ttu-id="28e15-673">此模式还有助于在缓存中保存的数据与原始数据存储中的数据之间保持一致性。</span><span class="sxs-lookup"><span data-stu-id="28e15-673">This pattern also helps to maintain consistency between data that's held in the cache and the data in the original data store.</span></span>

- <span data-ttu-id="28e15-674">[分片模式](../patterns/sharding.md)提供了有关实施水平分区，以帮助在存储和访问大量数据时提高伸缩性的信息。</span><span class="sxs-lookup"><span data-stu-id="28e15-674">The [Sharding pattern](../patterns/sharding.md) provides information about implementing horizontal partitioning to help improve scalability when storing and accessing large volumes of data.</span></span>

## <a name="more-information"></a><span data-ttu-id="28e15-675">详细信息</span><span class="sxs-lookup"><span data-stu-id="28e15-675">More information</span></span>

- [<span data-ttu-id="28e15-676">Azure Redis 缓存文档</span><span class="sxs-lookup"><span data-stu-id="28e15-676">Azure Redis Cache documentation</span></span>](/azure/azure-cache-for-redis/)
- [<span data-ttu-id="28e15-677">Azure Redis 缓存常见问题解答</span><span class="sxs-lookup"><span data-stu-id="28e15-677">Azure Redis Cache FAQ</span></span>](/azure/redis-cache/cache-faq)
- [<span data-ttu-id="28e15-678">基于任务的异步模式</span><span class="sxs-lookup"><span data-stu-id="28e15-678">Task-based Asynchronous pattern</span></span>](/dotnet/standard/asynchronous-programming-patterns/task-based-asynchronous-pattern-tap)
- [<span data-ttu-id="28e15-679">Redis 文档</span><span class="sxs-lookup"><span data-stu-id="28e15-679">Redis documentation</span></span>](https://redis.io/documentation)
- [<span data-ttu-id="28e15-680">StackExchange.Redis</span><span class="sxs-lookup"><span data-stu-id="28e15-680">StackExchange.Redis</span></span>](https://stackexchange.github.io/StackExchange.Redis/)
- [<span data-ttu-id="28e15-681">数据分区指南</span><span class="sxs-lookup"><span data-stu-id="28e15-681">Data partitioning guide</span></span>](https://msdn.microsoft.com/library/dn589795.aspx)
