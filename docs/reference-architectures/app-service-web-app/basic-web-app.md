...
此参考体系结构演示使用 [Azure 应用服务][app-service]和 [Azure SQL 数据库][sql-db]的 Web 应用程序适用的成熟做法。 [**部署此解决方案**](#deploy-the-solution)。
...
如果需要为 SQL 数据库使用更高的服务层或性能级别，可以纵向扩展单个数据库，且无需关闭应用程序。 有关详细信息，请参阅 [SQL 数据库选项和性能：了解每个服务层中有哪些可用资源][sql-db-scale]。
...
使用 [Azure DevOps][azure-devops]、[Visual Studio Team Foundation Server][tfs] 等工具执行负载测试。 有关云应用程序中性能分析的一般性概述，请参阅[性能分析入门][perf-analysis]。
