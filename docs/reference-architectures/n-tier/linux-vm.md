...
本文介绍经验证的做法，说明如何在 Azure 上运行 Linux 虚拟机 (VM)。 其中包括有关预配 VM 以及网络和存储组件的建议。 [**部署此解决方案**](#deploy-the-solution)。
...
1. 创建 SSH 密钥对。 有关详细信息，请参阅[如何创建和使用适用于 Azure 中 Linux VM 的 SSH 公钥和私钥对](/azure/virtual-machines/linux/mac-create-ssh-keys)。

2. 导航到已在前面的先决条件步骤中下载的存储库的 `virtual-machines/single-vm/parameters/linux` 文件夹。

3. 打开 `single-vm-v2.json` 文件并输入括在引号中的用户名和 SSH 公钥，然后保存文件。
