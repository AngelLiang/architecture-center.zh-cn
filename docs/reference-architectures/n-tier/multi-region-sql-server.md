...
---
# <a name="n-tier-application-in-multiple-azure-regions-for-high-availability"></a>通过位于多个 Azure 区域中的 N 层应用程序确保高可用性
...
此体系结构基于[使用 SQL Server 的 N 层应用程序](n-tier-sql-server.md)中所示的体系结构。 
...
以下 [Azure CLI][azure-cli] 命令更新优先级：
...
* 针对每个 VNet，将两个区域中的域控制器的 IP 地址添加到 DNS 服务器列表。 可以使用以下 CLI 命令。 有关详细信息，请参阅[更改 DNS 服务器][vnet-dns]。
