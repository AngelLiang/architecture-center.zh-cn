...
- **手动故障转移**。 某些系统无法自动故障转移，需要手动故障转移。 对于配置了 [Azure Site Recovery][site-recovery] 的 Azure 虚拟机，可以[执行故障转移][site-recovery-failover]并在几分钟内将虚拟机恢复到另一个区域中。
