...
3. 在 secrets.json 文件中，粘贴以下代码：
    ...
    - `AzureAd:WebApiResourceId`：在 Azure AD 中创建 Surveys.WebAPI 应用程序时指定的应用 ID URI。 它应当采用 `https://<directory>.onmicrosoft.com/surveys.webapi` 形式
...
7. 如果你具有 Azure AD Premium，请单击“用户和组”。 否则，请单击“用户”。 （将角色分配给组需要 Azure AD Premium。）
...
> [!IMPORTANT]
> 用户在 Survey 和 Survey.WebAPI 中应当始终具有相同的角色。 否则，用户将具有不一致的权限，这将导致 Web API 发出 403（被禁止）错误。
