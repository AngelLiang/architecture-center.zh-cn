...
此方案演示如何部署和运行使用单一登录 (SSO) 保护的 Azure 上的可缩放的企业级 Jenkins 服务器。 本体系结构还使用 Azure Monitor 来监视 Jenkins 服务器的状态。 [**部署此解决方案**](#deploy-the-solution)。
...
- **Jenkins 服务器**。 将虚拟机部署运行[Jenkins] [ azure-market]作为自动化服务器并充当 Jenkins 主节点。 若要部署此 VM，请使用[Azure 上的 Jenkins 解决方案模板][solution]。 Azure 市场中提供了其他 Jenkins 产品。
...
使用从 Azure 市场安装的 Jenkins [Windows Azure 存储插件][storage-plugin]能够存储可与其他生成和测试组件共享的生成项目。 必须先配置一个 Azure 存储帐户，然后 Jenkins 作业才能使用此插件。

### <a name="jenkins-azure-plugins"></a>Jenkins Azure 插件

Azure 上的 Jenkins 解决方案模板会安装多个 Azure 插件。 Azure DevOps 团队会生成并维护该解决方案模板和以下插件，这些插件使用 Azure 市场中的其他 Jenkins 产品，以及本地安装的 Jenkins 主节点：
...
- 使用 [Azure VM 代理][vm-agent]插件创建可在 Azure VM 中运行的 Jenkins 代理。 此插件可让代理实现弹性扩展，并可使用不同类型的虚拟机。 可以从 Azure 市场中选择不同的基础映像，也可以使用自定义映像。 有关 Jenkins 代理如何缩放的详细信息，请参阅 Jenkins 文档中的 [Architecting for Scale][scale]（构建可缩放的解决方案）。
...
1. 在 Web 浏览器中打开[用于 Jenkins 的 Azure 市场映像][azure-market]，然后从页面左侧选择“立即获取”。
